...
DELIMITER $$
create trigger insertLCS1 after insert on lcs2_mantenimiento.vehiculo for each row 
begin
...
DELIMITER $$
create trigger insertLCS3 after insert on lcs2_mantenimiento.vehiculo for each row 
begin
...
AFTER UPDATE ON lcs2_mantenimiento.vehiculo
FOR EACH ROW
BEGIN
...
BEGIN
    DELETE FROM lcs2_mantenimiento.vehiculo WHERE vehiculoid = vehid;
END $$
DELIMITER ;

DELIMITER $$
CREATE TRIGGER deleteLCS1 
AFTER DELETE ON lcs2_mantenimiento.vehiculo
FOR EACH ROW
BEGIN
